...
export class EleveService {
  ...
  getEleves(): Observable<Eleve[]> {
    return this.http.get<Eleve[]>(`${this.baseUrl}admin/view.php`);
  }
  ...
  deleteEleve(id: number): Observable<any> {
    const url = `${this.baseUrl}admin/delete.php?id=${id}`;
  ...
  createEleve(eleve: Eleve): Observable<any> {
    return this.http.post(`${this.baseUrl}admin/insert.php`, eleve); // Envoie de l'objet élève
  }

  // Modifier un élève existant
  editEleve(eleve: Eleve): Observable<any> {
    return this.http.put(`${this.baseUrl}admin/update.php`, eleve);
  }

  // Méthode pour récupérer les auto-écoles (si nécessaire)
  getAutoecoleList(): Observable<any> {
    return this.http.get<any>(`${this.baseUrl}auto-ecole/get_autoecoles.php`); // Assure-toi que cette API existe
  }
}